...
@Slf4j
public class BlockChainStorageBack {
    ...
    private static final String CF_UTXOS = "utxos";       // 存储区块
    ...
    private static final byte[] KEY_MINER = "miner".getBytes();
    ...
    private static class InstanceHolder {
        private static final BlockChainStorageBack INSTANCE = new BlockChainStorageBack();
    }


    public static BlockChainStorageBack getInstance() {
        return BlockChainStorageBack.InstanceHolder.INSTANCE;
    }

    private BlockChainStorageBack() {
        try {
    ...
    private RocksDB openRocksDBWithColumnFamilies() throws RocksDBException {
        ...
        // 3. 配置自定义列族（不存在则创建）
        for (String cfName : new String[]{CF_BLOCKS,CF_CHAINSTATE, CF_METADATA,CF_UTXOS}) {
        ...
        this.cfUtxo = cfHandles.get(4);           // 对应CF_UTXOS  UTXO集 key是ID  value是 UTXO集合
        return db;
    }
    ...
    public void putBlock(Block block) {
        ...
        try {
            ...
            //高度→区块哈希 索引
            db.put(cfMetadata, ByteUtils.toBytes(block.getHeight()), blockHash);
        } catch (RocksDBException e) {
    ...
    public Block getBlockByHeight(Long height){
        ...
        try {
            byte[] blockData = db.get(cfMetadata, ByteUtils.toBytes(height));
            if (blockData == null) {
    ...
    public Block getBlockByHeight(long height) {
        rwLock.readLock().lock();
        try {
            byte[] valueBytes = db.get(cfMetadata, ByteUtils.toBytes(height));
            if (valueBytes == null) {
                return null; // 不存在返回null，避免抛出异常
            }
            return (Block)SerializeUtils.deSerialize(valueBytes);
        } catch (RocksDBException e) {
    ...
    public Block getBlock(byte[] blockHash) {
        rwLock.readLock().lock();
        try {
            byte[] blockData = db.get(cfBlocks, blockHash);
            if (blockData == null) {
    ...
    public void addOrUpdateMiner(Miner value) {
        ...
        try {
            byte[] valueBytes = SerializeUtils.serialize(value);
            db.put(cfMetadata, KEY_MINER, valueBytes);
        } catch (RocksDBException e) {
    ...
    public Miner getMiner() {
        rwLock.readLock().lock();
        try {
            byte[] valueBytes = db.get(cfMetadata, KEY_MINER);
            if (valueBytes == null) {
                return null; // 不存在返回null，避免抛出异常
            }
