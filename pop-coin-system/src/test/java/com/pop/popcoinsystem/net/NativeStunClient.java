package com.pop.popcoinsystem.net;

import java.net.*;
import java.nio.ByteBuffer;

public class NativeStunClient {
    ...
    public static void main(String[] args) {
        try (DatagramSocket socket = new DatagramSocket()) {
            ...
            // 发送请求
            InetAddress serverAddress = InetAddress.getByName(STUN_SERVER);
            DatagramPacket packet = new DatagramPacket(request, request.length, serverAddress, STUN_PORT);
            socket.send(packet);
            System.out.println("已发送STUN请求到 " + STUN_SERVER + ":" + STUN_PORT);
            ...
            // 解析响应
            parseStunResponse(response, responsePacket.getLength());
    ...
    private static void parseStunResponse(byte[] data, int length) {
        ...
        while (remaining > 0 && buffer.hasRemaining()) {
            ...
            if (attrType == 0x0001) { // Mapped Address属性
                parseMappedAddress(buffer, attrLength);
            } else {
                // 对于其他属性，仅移动指针
    ...
    // 解析映射地址属性
    private static void parseMappedAddress(ByteBuffer buffer, int length) {
        if (length < 4) {
